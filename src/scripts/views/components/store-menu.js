class StoreMenu extends HTMLElement {
  ...
  fetchProfile() {
    ...
    if (!token) {
      ...
      fetch(`http://localhost:3000/api/farmers/${idFarm}`, {
        method: 'GET',
        headers: {
  ...
  async uploadProfileImage() {
    ...
    try {
      const response = await fetch('http://localhost:3000/api/farmers/uploadImage', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
  ...
  async updateEmail() {
    ...
    try {
      const response = await fetch(`http://localhost:3000/api/farmers/${idFarm}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
